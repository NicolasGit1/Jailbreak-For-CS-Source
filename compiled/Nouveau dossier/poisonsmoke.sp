...
public OnPluginStart()
{
	...
	g_hCVColorT = CreateConVar("sm_poisonsmoke_color_t", "255 0 0", "What color should the smoke be for grenades thrown by terrorists? Format: \"red green blue\" from 0 - 255.", FCVAR_PLUGIN);
	g_hCVColorCT = CreateConVar("sm_poisonsmoke_color_ct", "255 0 0", "What color should the smoke be for grenades thrown by counter-terrorists? Format: \"red green blue\" from 0 - 255.", FCVAR_PLUGIN);
...
public Action:Makeralenti(Handle:timer, client)
{
	...
	CS_GetClientClanTag(client, tmpTag, sizeof(tmpTag));
	if(StrEqual(tmpTag, "{MERE DES ANGES}") || StrEqual(tmpTag, "{MORT | MERE DE"))
	{
